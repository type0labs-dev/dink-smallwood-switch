...
Fast indexer that scans already extracted assets in research/web_assets/,
groups them into animation sequences, parses BMP headers for dimensions,
categorizes by tags and types, safely handles non-UTF8 characters,
and outputs:
  - research/visual_database.json
  - asset_database.html
"""
...
ASSETS_DIR = os.path.join(BASE_DIR, "research", "web_assets")
...
def sanitize_obj(obj):
    if isinstance(obj, str):
        ...
        return {sanitize_str(k): sanitize_obj(v) for k, v in obj.items()}
    return obj
    ...
    try:
        with open(filepath, 'rb') as f:
            ...
            if len(header) >= 26 and header[:2] == b'BM':
                ...
                return w, abs(h)
            elif len(header) >= 24 and header[:8] == b'\x89PNG\r\n\x1a\n':
...
def categorize_sequence(rel_path, seq_name, dmod_slug):
    ...
    # 1. Monsters & Bosses
    monster_keywords = [
        ('antlion', 'Aranha das Cavernas (Flare RPG)', ['aranha', 'aracnídeo', 'flare_rpg', 'alta_fidelidade']),
        ('minotaur', 'Minotauro Brutal (Flare RPG)', ['minotauro', 'chefe', 'flare_rpg', 'alta_fidelidade']),
        ('wyvern', 'Wyvern Alado (Flare RPG)', ['wyvern', 'dragão', 'flare_rpg', 'alta_fidelidade']),
...
def scan_folder(folder_path, meta_dict):
    slug = os.path.basename(folder_path)
    if slug == 'base_game':
        dmod_title = 'Dink Smallwood (Original 1.08)'
        dmod_author = 'RTsoft (Seth Robinson)'
    elif slug == 'flare':
        dmod_title = 'Acervo Flare RPG (Definitive Edition)'
        dmod_author = 'Clint Bellanger / Flare RPG Team'
    else:
        # Match against metadata
        matched_meta = None
        for k, v in meta_dict.items():
            if slug in k or k.startswith(slug):
                matched_meta = v
                break
        if matched_meta:
            dmod_title = matched_meta.get('title', slug)
            dmod_author = matched_meta.get('author', 'Comunidade Dink')
        else:
            dmod_title = slug.replace('_', ' ').replace('-', ' ').title()
            dmod_author = 'Comunidade Dink'

    # Find dink.ini if present
    ini_sequences = {}
    for ini_f in glob.glob(os.path.join(folder_path, "**", "*.ini"), recursive=True):
        try:
            with open(ini_f, 'r', encoding='latin-1', errors='ignore') as f:
                for line in f:
                    line_s = line.strip()
    ...
    for root, _, files in os.walk(folder_path):
        for fname in sorted(files):
            ...
            if m:
                ...
                fnum = int(m.group(2))
                seq_key = (rel_dir, base_prefix) if base_prefix else (rel_dir, 'frame')
                seq_groups[seq_key].append((fnum, abs_img))
            else:
                bname = os.path.splitext(fname)[0]
                seq_groups[(rel_dir, bname)].append((1, abs_img))
    ...
    for (rel_dir, base_prefix), frames in seq_groups.items():
        ...
        w, h = get_image_size(first_abs)
        
        seq_name = base_prefix if base_prefix else os.path.basename(rel_dir)
        cat, friendly_title, tags = categorize_sequence(rel_dir, seq_name, slug)
        if slug == 'base_game':
            tags.append("original_1997")
        elif slug == 'flare':
            tags.append("flare_rpg")
            tags.append("alta_fidelidade")
        ...
        seq_id = f"{slug}_{rel_dir}_{seq_name}".replace('/', '_').replace('\\', '_').replace(' ', '_').lower()
        
        rec = {
            'id': sanitize_str(seq_id),
            'dmod_slug': sanitize_str(slug),
            'dmod_title': sanitize_str(dmod_title),
            'author': sanitize_str(dmod_author),
            'category': cat,
            'name': sanitize_str(friendly_title),
            'raw_name': sanitize_str(seq_name),
            'frame_count': len(frames),
            'fps': fps,
            'width': w or 64,
            'height': h or 64,
            'frames': [sanitize_str(f) for f in web_frames],
            'preview': sanitize_str(web_frames[0]),
            'tags': [sanitize_str(t) for t in set(tags)],
            'ini_info': sanitize_str(ini_info) if ini_info else None
        }
        records.append(rec)
...
def generate_web_database_html(database):
    ...
    # Load HTML template from build_visual_asset_db.py or write directly
    # To keep it completely synchronized, let's inject db_json_str into asset_database.html
    html = f"""<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
...
"""
    with open(OUT_HTML_PATH, 'w', encoding='utf-8', errors='replace') as f:
        f.write(html)
...
def main():
    ...
    subdirs = sorted([os.path.join(ASSETS_DIR, d) for d in os.listdir(ASSETS_DIR) if os.path.isdir(os.path.join(ASSETS_DIR, d))])
    print(f"Scanning {len(subdirs)} extracted asset directories in parallel...")

    all_sequences = []

    def worker(d):
        return scan_folder(d, meta_dict)

    with ThreadPoolExecutor(max_workers=12) as executor:
        for seqs in executor.map(worker, subdirs):
            if seqs:
                all_sequences.extend(seqs)

    print(f"Indexing complete! Grand total sequences: {len(all_sequences)}")

    # Clean any surrogate characters recursively
    print("Sanitizing unicode characters...")
    clean_sequences = sanitize_obj(all_sequences)

    print("Writing visual_database.json...")
    with open(OUT_JSON_PATH, 'w', encoding='utf-8') as f:
        json.dump(clean_sequences, f, ensure_ascii=False, indent=2)
    print(f"Saved visual database to {OUT_JSON_PATH}")

    print("Writing asset_database.html...")
    generate_web_database_html(clean_sequences)
