...
// Criatura: Goblin Guerreiro de Elite (goblin_elite)
// Descricao: Campeão do clã goblin com elmo e escudo reforçado.
// ========================================================
...
void main( void )
{
    ...
    sp_base_walk(&current_sprite, 1020);
    sp_base_attack(&current_sprite, 1030);
    sp_base_death(&current_sprite, 1040);
...
void hit( void )
{
    ...
    if (&mhold == 1)
    {
        say("Pela glória do grande chefe!", &current_sprite);
    }
    if (&mhold == 2 && 2 > 1)
    {
        say("Escudo forte!", &current_sprite);
    }
}
...
void die( void )
{
    ...
    say("O clã... vingará...", &current_sprite);
