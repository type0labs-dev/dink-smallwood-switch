...
// Criatura: Formiga de Gelo Cristalina (ice_ant)
// Descricao: Inseto ártico com mandíbulas afiadas de gelo puro.
// ========================================================
...
void main( void )
{
    ...
    sp_base_walk(&current_sprite, 990);
    sp_base_attack(&current_sprite, 1000);
    sp_base_death(&current_sprite, 1010);
...
void hit( void )
{
    ...
    if (&mhold == 1)
    {
        say("*Estilhaços de gelo!*", &current_sprite);
    }
    if (&mhold == 2 && 2 > 1)
    {
        say("*Frio cortante!*", &current_sprite);
    }
}
