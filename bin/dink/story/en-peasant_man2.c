...
// Criatura: Fazendeiro da Vila (peasant_man2)
// Descricao: Veterano dos campos, conhecedor dos segredos da região.
// ========================================================
...
void main( void )
{
    ...
    sp_base_walk(&current_sprite, 1230);
    sp_base_attack(&current_sprite, 1240);
    sp_base_death(&current_sprite, 1250);
...
void hit( void )
{
    ...
    if (&mhold == 1)
    {
        say("Malditos monstros atacando meus porcos!", &current_sprite);
    }
    if (&mhold == 2 && 2 > 1)
    {
        say("O reino já foi mais seguro.", &current_sprite);
    }
}
...
void die( void )
{
    ...
    say("Adeus, boa terra...", &current_sprite);
