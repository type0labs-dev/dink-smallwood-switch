...
// Criatura: Esqueleto Necromante (skeleton_mage)
// Descricao: Conjurador ósseo mestre das trevas que canaliza maldições.
// ========================================================
...
void main( void )
{
    ...
    sp_base_walk(&current_sprite, 880);
    sp_base_attack(&current_sprite, 890);
    sp_base_death(&current_sprite, 900);
...
void hit( void )
{
    ...
    if (&mhold == 1)
    {
        say("As sombras consomem você!", &current_sprite);
    }
    if (&mhold == 2 && 2 > 1)
    {
        say("Sinta o frio do além!", &current_sprite);
    }
}
...
void die( void )
{
    ...
    say("A morte é apenas o começo...", &current_sprite);
