...
// Criatura: Tumba Amaldiçoada (cursed_grave)
// Descricao: Sepultura profana que invoca esqueletos continuamente até ser destruída.
// ========================================================
...
void main( void )
{
    ...
    sp_base_walk(&current_sprite, 1180);
    sp_base_attack(&current_sprite, 1190);
    sp_base_death(&current_sprite, 1200);
...
void hit( void )
{
    ...
    if (&mhold == 1)
    {
        say("*Pedra da tumba range!*", &current_sprite);
    }
    if (&mhold == 2 && 2 > 1)
    {
        say("*Espíritos clamam!*", &current_sprite);
    }
}
...
void die( void )
{
    ...
    say("*A sepultura se quebra e a maldição cessa.*", &current_sprite);
