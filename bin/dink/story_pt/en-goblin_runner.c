...
// Criatura: Goblin Rastreador (goblin_runner)
// Descricao: Batedor ultra-rápido que cerca Dink antes que ele possa reagir.
// ========================================================
...
void main( void )
{
    ...
    sp_exp(&current_sprite, 22);
    
    sp_base_walk(&current_sprite, 1010);
    sp_base_attack(&current_sprite, 1020);
    sp_base_death(&current_sprite, 1030);
...
void hit( void )
{
    ...
    if (&mhold == 1)
    {
        say("Você é muito lento!", &current_sprite);
    }
    if (&mhold == 2 && 2 > 1)
    {
        say("Pega se puder!", &current_sprite);
    }
}
