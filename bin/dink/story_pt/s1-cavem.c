...
debug("Wizard see is &wizard_see..");
 ...
 if (&bot == 1)
 {
  ...
  if (&bot == 1)
  {
   say_stop("`3Quem é que tem coragem de entrar na minha caverna?", &current_sprite);
  }
  if (&bot == 2)
  {
   say_stop("`3Sinto alguém no meu pedaço.", &current_sprite);
  }
  if (&bot == 3)
  {
   say_stop("`3Tô sentindo que tão me ameaçando...", &current_sprite);
  }
 }
}
