...
// Criatura: Matrona da Vila (peasant_woman2)
// Descricao: Cozinheira e curandeira respeitada da aldeia.
// ========================================================
...
void main( void )
{
    ...
    sp_base_walk(&current_sprite, 1250);
    sp_base_attack(&current_sprite, 1260);
    sp_base_death(&current_sprite, 1270);
...
void hit( void )
{
    ...
    if (&mhold == 1)
    {
        say("Menino mal-educado!", &current_sprite);
    }
    if (&mhold == 2 && 2 > 1)
    {
        say("Vou te dar um corretivo!", &current_sprite);
    }
}
...
void die( void )
{
    ...
    say("Quem cuidará da sopa...", &current_sprite);
