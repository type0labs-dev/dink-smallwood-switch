...
//set starting pic
 ...
 say_stop("`2Beleza, é aqui em cima", &current_sprite);
 move_stop(&current_sprite, 8, 200, 1);
 say_stop("`2Vem comigo...", &current_sprite);
 ...
 wait(100);
 playsound(19, 44102, 0, 0, 0);
 say("`2Agora, se eu conseguir abrir essa fechadura...", &current_sprite);
 move_stop(1, 8, 190, 1);
 move_stop(1, 6, 350, 1);
 say_stop("O que a gente tá fazendo?", 1);
 playsound(19, 44102, 0, 0, 0);
 wait(100);
 playsound(19, 44102, 0, 0, 0);
 say_stop("`2Só mais um segundo...", &current_sprite);
 playmidi("battle.mid");
 //build guards
 preload_seq(291);
 preload_seq(293);
 ...
 say_stop("`2Ah não, os guardas!!  Vaza daqui!", &current_sprite);
 wait(250);
 sp_dir(1, 2);
 &thief = 3;
 say_stop("Isso não tá bom.", 1);
 unfreeze(1);
 unfreeze(&jcrap);
 unfreeze(&crap);
