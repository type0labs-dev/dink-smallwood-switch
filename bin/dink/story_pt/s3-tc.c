...
sp_script(&pp9, "s3-peeps");
  ...
  move(&mp7, 4, -1500, 1);
  wait(500);
  say_stop("`9Dink, Dink, vem cá.", &woman);
  move_stop(1, 2, 105, 1);
  move_stop(1, 6, 450, 1);
  say_stop("`9Não é lindo?", &woman);
  sp_dir(1, 4);
  wait(250);
  say_stop("É, é um desfile, tá certinho.", 1);
  wait(250);
  say_stop("Muita gente também, me arrepio só de pensar no que podia ter acontecido.", 1);
  wait(250);
  say_stop("`9Você salvou a cidade mesmo, Dink...", &woman);
  wait(250);
  say_stop("`9Tô muito orgulhosa de você.", &woman);
  wait(250);
  say_stop("Valeu, mas sem você eu não teria conseguido.", 1);
  wait(250);
  say_stop("`9Bom, preciso ir, tenho que encontrar com meu pai.", &woman);
  wait(250);
  say_stop("`9Se cuida, Dink, espero te ver de novo.", &woman);
  wait(1000);
  unfreeze(&pp1);
  unfreeze(&pp2);
  unfreeze(&pp3);
  ...
  say_stop("Ah, vai me ver sim, gata, vai sim...", 1);
  &story = 10;
  &mayor = 6;
  unfreeze(1);
  return;
