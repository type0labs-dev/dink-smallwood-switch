...
//    sp_dir(&current_sprite, 0);
      ...
      sp_pframe(&current_sprite, 1);
      wait(200);      
        say("`4<Lança explosão>", &current_sprite);
...
if (&resist > &magic)
        {
        say("`4<Lança dano maior>", &current_sprite);
...
if (&dam < 6)
  {
   say("`%Assiste o Conan O'brien!", &CURRENT_SPRITE);
   return;
  }

if (&dam < 15)
  {
   say("`%MÃEEEEEEEEEEEEEEEE!", &CURRENT_SPRITE);
   return;
  }
...
if (&dam < 50)
  {
   say("`%SINTA A MINHA IRA, SEU PEQUENO INSETO!", &CURRENT_SPRITE);
   return;
  }

if (&dam < 100)
  {
   say("`%Isso até doeu.", &CURRENT_SPRITE);
  return;
  }
...
if (&dam < 140)
  {
   say("`%Escuta os passarinhos... até eles tão pedindo o seu coração.", &CURRENT_SPRITE);
...
if (&dam < 160)
  {
   say("`%Você vai bater as botas logo logo.", &CURRENT_SPRITE);
  return;
  }
...
if (&dam < 180)
  {
   say("`%Bela tacada. Mas vai ser a última.", &CURRENT_SPRITE);
  return;
  }

if (&dam < 200)
  {
   say("`%Tá começando a me irritar, seu chato.", &CURRENT_SPRITE);
    return;
  }
...
if (&dam < 240)
  {
   say("`%Eu podia te acabar agora, mas isso tá me divertindo pra caralho.", &CURRENT_SPRITE);
  return;
  }
...
if (&dam < 260)
  {
   say("`%É meio triste você morrer sem motivo nenhum.", &CURRENT_SPRITE);
  return;
  }
...
if (&dam < 280)
  {
   say("`%Isso até fez cócegas.", &CURRENT_SPRITE);
  return;
  }


   say("`%Você é igualzinho a mim, sabia, Dink?", &CURRENT_SPRITE);
...
void die( void )
{
 ...
 say_stop("Pena que tive que matar ele, parecia até gente boa.", 1);
 wait(500);
 say_stop("Meu corpo tá formigando!", 1);
 &life = &lifemax;
 wait(500);
 say_stop("Agora preciso voltar pro Castelo Goodheart e informar o Rei.", 1);
 unfreeze(1);
 &story = 15;
