...
// Criatura: Wyvern das Tempestades (wyvern_air)
// Descricao: Dragão elétrico que canaliza relâmpagos entre as asas.
// ========================================================
...
void main( void )
{
    ...
    sp_base_walk(&current_sprite, 1100);
    sp_base_attack(&current_sprite, 1110);
    sp_base_death(&current_sprite, 1120);
    
    sp_touch_damage(&current_sprite, 13);
    sp_hitpoints(&current_sprite, 95);
...
void hit( void )
{
    ...
    if (&mhold == 1)
    {
        say("*Relâmpagos estalam no ar!*", &current_sprite);
    }
    if (&mhold == 2 && 2 > 1)
    {
        say("*Trovoada feroz!*", &current_sprite);
    }
}
...
void die( void )
{
    ...
    say("*O relâmpago se extingue...*", &current_sprite);
