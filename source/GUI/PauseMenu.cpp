...
Entity * PauseMenuCreate(Entity *pParentEnt)
{
	...
	pButtonEntity = CreateTextButtonEntity(pTextBG, "Map", x, y, bPT ? "VER MAPA" : "VIEW MAP", false); y += ySpacer;
	pButtonEntity->GetShared()->GetFunction("OnButtonSelected")->sig_function.connect(&PauseMenuOnSelect);
	pButtonEntity->GetVar("alignment")->Set(uint32(ALIGNMENT_CENTER));
	...
	pButtonEntity = CreateTextButtonEntity(pTextBG, "OldOptions", x, y, bPT ? "MENU DINK" : "DINK MENU", false); y += ySpacer;
	...
	{
		pButtonEntity = CreateTextButtonEntity(pTextBG, "Keyboard", x, y, bPT ? "TECLADO" : "FULL KEYBOARD", false); y += ySpacer;
		pButtonEntity->GetShared()->GetFunction("OnButtonSelected")->sig_function.connect(&PauseMenuOnSelect);
		pButtonEntity->GetVar("alignment")->Set(uint32(ALIGNMENT_CENTER));
		SetupTextEntity(pButtonEntity, fontID,fontScale);
	}

	pButtonEntity = CreateTextButtonEntity(pTextBG, "Quit", x, y, bPT ? "SAIR" : "QUIT", false); y += ySpacer;
	pButtonEntity->GetShared()->GetFunction("OnButtonSelected")->sig_function.connect(&PauseMenuOnSelect);
	pButtonEntity->GetVar("alignment")->Set(uint32(ALIGNMENT_CENTER));
	...
	pButtonEntity = CreateTextButtonEntity(pTextBG, "Options", x, y, bPT ? "OPCOES" : "OPTIONS", false); y += ySpacer;
	pButtonEntity->GetShared()->GetFunction("OnButtonSelected")->sig_function.connect(&PauseMenuOnSelect);
	pButtonEntity->GetVar("alignment")->Set(uint32(ALIGNMENT_CENTER));
	SetupTextEntity(pButtonEntity, fontID,fontScale);

	pButtonEntity = CreateTextButtonEntity(pTextBG, "Back", x, y, bPT ? "CONTINUAR" : "CONTINUE", false); y += ySpacer;
	pButtonEntity->GetShared()->GetFunction("OnButtonSelected")->sig_function.connect(&PauseMenuOnSelect);
	pButtonEntity->GetVar("alignment")->Set(uint32(ALIGNMENT_CENTER));
	SetupTextEntity(pButtonEntity, fontID, fontScale);
