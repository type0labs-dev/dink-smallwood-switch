...
void OptionsMenuOnSelect(VariantList *pVList) //0=vec2 point of click, 1=entity sent from
{
	...
	if (pEntClicked->GetName() == "lang_0")
	{
		GetApp()->GetShared()->GetVar("language")->Set(uint32(0));
		SetupLightBarSelect(pMenu, "lang_", 0, MAKE_RGBA(190, 0, 35, 255));
	}
	if (pEntClicked->GetName() == "lang_1")
	{
		GetApp()->GetShared()->GetVar("language")->Set(uint32(1));
		SetupLightBarSelect(pMenu, "lang_", 1, MAKE_RGBA(190, 0, 35, 255));
	}
...
void OptionsMenuAddScrollContent(Entity *pParent)
{
	...
	*/
	...
	#ifdef __SWITCH__
	//fps limit
	pEnt = CreateTextLabelEntity(pBG, "", startX, y, "Frame rate:");
	SetupTextEntity(pEnt,fontID);
	offsetX =  iPhoneMapX(columnX);
	pEnt = CreateTextButtonEntity(pBG, "fps_limit_0", offsetX, y, "30 FPS", false);
	SetupTextEntity(pEnt,fontID);
	pEnt->GetFunction("OnButtonSelected")->sig_function.connect(&OptionsMenuOnSelect);
	offsetX += pEnt->GetVar("size2d")->GetVector2().x + spacerX;

	pEnt = CreateTextButtonEntity(pBG, "fps_limit_1", offsetX, y, "60 FPS", false);
	SetupTextEntity(pEnt,fontID);
	pEnt->GetFunction("OnButtonSelected")->sig_function.connect(OptionsMenuOnSelect);
	offsetX += pEnt->GetVar("size2d")->GetVector2().x + spacerX;
	...
	y += GetSize2DEntity(pEnt).y + spacerY;
	#endif

	// Language / Idioma
	pEnt = CreateTextLabelEntity(pBG, "", startX, y, "Language / Idioma:");
	SetupTextEntity(pEnt, fontID);
	offsetX = iPhoneMapX(columnX);
	pEnt = CreateTextButtonEntity(pBG, "lang_1", offsetX, y, "Português", false);
	SetupTextEntity(pEnt, fontID);
	pEnt->GetFunction("OnButtonSelected")->sig_function.connect(&OptionsMenuOnSelect);
	offsetX += pEnt->GetVar("size2d")->GetVector2().x + spacerX;

	pEnt = CreateTextButtonEntity(pBG, "lang_0", offsetX, y, "English", false);
	SetupTextEntity(pEnt, fontID);
	pEnt->GetFunction("OnButtonSelected")->sig_function.connect(OptionsMenuOnSelect);
	offsetX += pEnt->GetVar("size2d")->GetVector2().x + spacerX;

	uint32 langID = GetApp()->GetVarWithDefault("language", uint32(1))->GetUINT32();
	SetupLightBarSelect(pBG, "lang_", langID, MAKE_RGBA(190, 0, 35, 255));
	y += GetSize2DEntity(pEnt).y + spacerY;
