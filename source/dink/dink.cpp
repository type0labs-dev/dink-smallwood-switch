...
#include "../../switch/ScreenLoadProfile.h"
#include "../../switch/SafeSave.h"
#include "../../switch/PerformancePolicy.h"
#include "../../switch/PathCache.h"
#include <switch.h>
#define DINK_PROFILE_SCREEN_LOAD(stage) SwitchScreenLoadProfile screenLoadProfile(stage)
#else
#define DINK_PROFILE_SCREEN_LOAD(stage)
#endif
#include <time.h>
...
string GetFileLocationString(const string fName)
{
...
#ifdef __SWITCH__
	string cachedLoc;
	if (dink_path_cache::lookupLocation(fName, cachedLoc))
	{
		return cachedLoc;
	}
#endif
	...
	if (!g_dglo.m_dmodGameDir.empty())
	{
		...
		if (FileExists(g_dglo.m_dmodGamePathWithDir+fName))
		{
			//found it
#ifdef __SWITCH__
			dink_path_cache::storeLocation(fName, g_dglo.m_dmodGamePathWithDir+fName);
#endif
	...
	if (GetEmulatedPlatformID() == PLATFORM_ID_IOS || GetEmulatedPlatformID() == PLATFORM_ID_OSX || GetEmulatedPlatformID() == PLATFORM_ID_HTML5)
	{
		...
		if (FileExists(g_dglo.m_gameDir+fName))
		{
			//found it
#ifdef __SWITCH__
			dink_path_cache::storeLocation(fName, g_dglo.m_gameDir+fName);
#endif
...
#ifdef __SWITCH__
	dink_path_cache::storeLocation(fName, g_dglo.m_gamePathWithDir + fName);
#endif
...
{
	DINK_PROFILE_SCREEN_LOAD("load_map (read, hardness, music)");
	//FILE *          fp;
	#ifndef __SWITCH__
	int holdme,lsize;
	#endif
...
#ifdef __SWITCH__
	// Loose SD files support direct seeks; compressed DMOD streams still need
	// sequential reads. Never discard the entire preceding map data on the SD.
	const bool canSeek = dynamic_cast<StreamingInstanceFile*>(pFile) != NULL;
	bool mapRead;
	{
		DINK_PROFILE_SCREEN_LOAD("map record read");
		mapRead = dink::ReadMapRecord(*pFile, num, canSeek, g_dglos.g_smallMap);
	}
	delete pFile;
	if (!mapRead)
	{
		LogError("Unable to read map record %d from %s", num, g_dglos.current_map);
		return;
	}
#else
...
#ifdef __SWITCH__
    if (!dink_save::write(crap, [](FILE* file) {
        return fwrite(&g_dglos.g_playerInfo, sizeof(g_dglos.g_playerInfo), 1, file) == 1;
    })) {
        LogError("Save slot %d failed", num);
        ShowQuickMessage("Save failed.");
        return;
    }
#else
...
void EnsureStackGlobals()
{
	if (var_exists((char*)"&nuts_total", 0) == 0)
		make_int((char*)"&nuts_total", 0, 0, 0);
	if (var_exists((char*)"&bombs_total", 0) == 0)
		make_int((char*)"&bombs_total", 0, 0, 0);
	if (var_exists((char*)"&elixir_total", 0) == 0)
		make_int((char*)"&elixir_total", 0, 0, 0);

	for (int it = 1; it <= C_DINK_MAX_ITEMS; it++)
	{
		if (g_dglos.g_playerInfo.g_itemData[it].active)
		{
			char* iName = g_dglos.g_playerInfo.g_itemData[it].name;
			if (strcasecmp(iName, "item-nut") == 0)
			{
				int v = get_var(0, (char*)"&nuts_total");
				if (v > 0 && g_dglos.g_playerInfo.var[v].active && g_dglos.g_playerInfo.var[v].var <= 0)
					g_dglos.g_playerInfo.var[v].var = 1;
			}
			else if (strcasecmp(iName, "item-bom") == 0)
			{
				int v = get_var(0, (char*)"&bombs_total");
				if (v > 0 && g_dglos.g_playerInfo.var[v].active && g_dglos.g_playerInfo.var[v].var <= 0)
					g_dglos.g_playerInfo.var[v].var = 1;
			}
			else if (strcasecmp(iName, "item-eli") == 0)
			{
				int v = get_var(0, (char*)"&elixir_total");
				if (v > 0 && g_dglos.g_playerInfo.var[v].active && g_dglos.g_playerInfo.var[v].var <= 0)
					g_dglos.g_playerInfo.var[v].var = 1;
			}
		}
...
bool load_game(int num)
{
	...
	else
	{
		...
		*pupdate_status = 1;
		...
		// Ensure stack counts are initialized on save load
		EnsureStackGlobals();
...
void kill_cur_item( void )
{
	...
	if (*pcur_weapon != 0) 
	{
		...
		{
			char killedName[32];
			strcpy_safe(killedName, g_dglos.g_playerInfo.g_itemData[*pcur_weapon].name);
			...
			*pcur_weapon = 0;
			...
			if (strcasecmp(killedName, "item-nut") == 0)
			{
				int v = get_var(0, (char*)"&nuts_total");
				if (v > 0) g_dglos.g_playerInfo.var[v].var = 0;
			}
			else if (strcasecmp(killedName, "item-bom") == 0)
			{
				int v = get_var(0, (char*)"&bombs_total");
				if (v > 0) g_dglos.g_playerInfo.var[v].var = 0;
			}
			else if (strcasecmp(killedName, "item-eli") == 0)
			{
				int v = get_var(0, (char*)"&elixir_total");
				if (v > 0) g_dglos.g_playerInfo.var[v].var = 0;
			}
...
void draw_icons()
{
	...
	if (*pcur_weapon != 0) if (g_dglos.g_playerInfo.g_itemData[*pcur_weapon].active)
	{
		...
		if (g_dglos.g_playerInfo.g_itemData[*pcur_weapon].active)
		{
			int count = 0;
			char* itemName = g_dglos.g_playerInfo.g_itemData[*pcur_weapon].name;
			if (strcasecmp(itemName, "item-nut") == 0)
			{
				int varIdx = get_var(0, (char*)"&nuts_total");
				count = (varIdx > 0 && g_dglos.g_playerInfo.var[varIdx].active) ? g_dglos.g_playerInfo.var[varIdx].var : 0;
			}
			else if (strcasecmp(itemName, "item-bom") == 0)
			{
				int varIdx = get_var(0, (char*)"&bombs_total");
				count = (varIdx > 0 && g_dglos.g_playerInfo.var[varIdx].active) ? g_dglos.g_playerInfo.var[varIdx].var : 0;
			}
			else if (strcasecmp(itemName, "item-eli") == 0)
			{
				int varIdx = get_var(0, (char*)"&elixir_total");
				count = (varIdx > 0 && g_dglos.g_playerInfo.var[varIdx].active) ? g_dglos.g_playerInfo.var[varIdx].var : 0;
			}
			if (count > 1)
			{
...
int load_script(const char *pScript, int sprite, bool set_sprite, bool bQuietError)
{
	...
	if (!g_dglo.m_dmodGameDir.empty())
	{
		...
		if (GetApp()->GetVarWithDefault("language", uint32(1))->GetUINT32() == 1)
		{
			string ptName = g_dglo.m_dmodGamePathWithDir + "story_pt/" + ToLowerCaseString(pScript) + ".c";
			if (FileExists(ptName))
			{
				fileName = ptName;
				bFound = true;
				comp = false;
	...
	if (!bFound)
	{
		if (GetApp()->GetVarWithDefault("language", uint32(1))->GetUINT32() == 1)
		{
			string ptDir = g_dglo.m_bUsingDinkPak ? g_dglo.m_gameDir : g_dglo.m_gamePathWithDir;
			string ptName = ptDir + "story_pt/" + ToLowerCaseString(pScript) + ".c";
			if (FileExists(ptName))
			{
				fileName = ptName;
				bFound = true;
...
{
#ifdef __SWITCH__
    // Remember the requested music even if its decoder or output is unavailable.
    // StopMidi explicitly clears it; a save must not turn a playback failure into silence.
    const string requestedMusic = sFileName;
    g_dglo.m_lastMusicPath = requestedMusic;
    sFileName = requestedMusic.c_str();
#endif
...
#ifdef __SWITCH__
    dink_music_diagnostics::write("RESOLVE request=%s resolved=%s exists=%d base=%s dmod=%s", sFileName, finalPath.c_str(), FileExists(finalPath), g_dglo.m_gamePathWithDir.c_str(), g_dglo.m_dmodGameDir.c_str());
#endif
	if (FileExists(finalPath))
	{
		AudioHandle musicHandle = GetAudioManager()->Play(finalPath, true, true, false);
#ifdef __SWITCH__
		if (musicHandle == AUDIO_HANDLE_BLANK) {
			LogError("Music did not start: %s", finalPath.c_str());
			return false;
		}
#endif
...
#ifdef __SWITCH__
static string GetLocalMusicRecoveryTrack()
{
    if (!pmap || *pmap < 1 || *pmap > 768) return "";
    FILE* file = fopen((GetSavePath()+"music-recovery.txt").c_str(), "rb");
    if (!file) return "";
    char text[4097];
    const size_t bytes = fread(text, 1, sizeof(text), file);
    const bool failed = ferror(file) != 0;
    fclose(file);
    if (failed || bytes > 4096) return "";
    return dink_music::recoveryTrack(string(text, bytes), *pmap, g_MapInfo.music[*pmap],
                                    g_dglos.midi_active, g_dglo.m_dmodGameDir.empty(),
                                    g_dglo.m_lastMusicPath);
}
#endif

#ifdef __SWITCH__
static void RecoverMusicAfterLoad()
{
    // LoadSoundState runs before attach() binds player_map. Defer any map-based
    // recovery until all save sections loaded successfully and attach completed.
    const string recovered = GetLocalMusicRecoveryTrack();
    if (!recovered.empty()) {
        dink_music_diagnostics::write("RECOVER after attach screen=%d track=%s", *pmap, recovered.c_str());
        PlayMidi(recovered.c_str());
    }
}
#endif
...
#ifdef __SWITCH__
    dink_music_diagnostics::write("MAP screen=%d music=%d active=%d", *pmap, g_MapInfo.music[*pmap], g_dglos.midi_active);
#endif
	if (!g_dglos.midi_active) return;
#ifdef __SWITCH__
    if (g_MapInfo.music[*pmap] == 0 && GetAudioManager()->GetMusicEnabled() &&
        !GetAudioManager()->IsPlayingMusic()) {
        string resume = g_dglo.m_lastMusicPath;
        if (resume.empty()) resume = GetLocalMusicRecoveryTrack();
        if (!resume.empty()) {
            dink_music_diagnostics::write("RESUME inherited screen=%d track=%s", *pmap, resume.c_str());
            PlayMidi(resume.c_str());
        }
    }
#endif
...
{
	DINK_PROFILE_SCREEN_LOAD("background and scripts");
	rtRect32                rcRect;
	int pa, cool;   
...
{
	DINK_PROFILE_SCREEN_LOAD("transition capture");
    ...
    if (sjoy.button[2] == true)
    {
        if (!run_through_tag_list_talk(h))
        {
			...
			
            {
                ...
                bool bPT = (GetApp()->GetVarWithDefault("language", uint32(1))->GetUINT32() == 1);
                if (bPT)
                {
                    if (randy == 1) say_text((char*)"`$Nao vejo nada aqui.",h,0);
                    if (randy == 2) say_text((char*)"`$Hein?",h,0);
                    if (randy == 3) say_text((char*)"`$Tenho certeza que nao posso falar com isso.",h,0);
                    if (randy == 4) say_text((char*)"`$O que?",h,0);
                    if (randy == 5) say_text((char*)"`$Que tedio...",h,0);
                    if (randy == 6) say_text((char*)"`$Nada acontecendo por aqui.",h,0);
                }
                else
                {
    ...
    if (sjoy.button[3] == true)
    {
        if (g_dglos.magic_script == 0)
        {
			...
			
            {
                ...
                if (bPT)
                {
                    if (randy == 1) say_text((char*)"`$Nao conheco nenhuma magia.",h,0);
                    if (randy == 2) say_text((char*)"`$Eu nao sou mago!",h,0);
                    if (randy == 3) say_text((char*)"`$Preciso aprender magia antes de tentar isso.",h,0);
                    if (randy == 4) say_text((char*)"`$Estou gesticulando feito um bobo a toa!",h,0);
                    if (randy == 5) say_text((char*)"`$Nada aconteceu.",h,0);
                    if (randy == 6) say_text((char*)"`$Abracadabra!",h,0);
                }
                else
                {
...
void draw_item(int num, bool magic, int mseq, int mframe)
{
    ...
    if (!magic && mseq != 423 && num >= 1 && num <= C_DINK_MAX_ITEMS && g_dglos.g_playerInfo.g_itemData[num].active)
    {
        int count = 0;
        char* itemName = g_dglos.g_playerInfo.g_itemData[num].name;
        if (strcasecmp(itemName, "item-nut") == 0)
        {
            int varIdx = get_var(0, (char*)"&nuts_total");
            count = (varIdx > 0 && g_dglos.g_playerInfo.var[varIdx].active) ? g_dglos.g_playerInfo.var[varIdx].var : 0;
        ...
        if (count > 1)
        {
            char countStr[16];
            snprintf(countStr, sizeof(countStr), "%d", count);
            draw_num(442, countStr, (int)r.left + 24, (int)r.top + 24);
        }
    }
}
...
void InitDinkPaths(string gamePath, string gameDir, string dmodGameDir)
{
    ...
    if (!dink_save::recover(g_dglo.m_savePath))
        LogError("Save recovery could not complete in %s", g_dglo.m_savePath.c_str());
#endif
}
...
{
#ifdef __SWITCH__
    // In application mode, allow up to 300 MiB of texture/surface cache to prevent
    // thrashing and SD card reloading during backtracking. In constrained applet
    // mode, retain the strict 32 MiB ceiling.
    static const int high = (appletGetAppletType() == AppletType_Application ? 300 : 32) * 1024 * 1024;
    static const int low = high * 8 / 9;
    static dink_perf::CachePressure pressure;
    static uint32 nextScan = 0;
    const uint32 now = GetBaseApp()->GetGameTick();
    if (!pressure.update(GetBaseApp()->GetMemUsed(), GetBaseApp()->GetTexMemUsed(), high, low)) return;
    const uint32 scanClock = SDL_GetTicks(); // Loading a save may rewind the game clock.
    if (nextScan && int32(scanClock-nextScan) < 0) return;
    nextScan = scanClock + 100; // Bound sorting/collection frequency under pressure.
    std::vector<dink_perf::CacheEntry> candidates;
    for (int i=1; i<C_MAX_SPRITES_ADDRESSABLE; ++i)
        if (g_pSpriteSurface[i]) candidates.push_back({size_t(i), dink_perf::age(now, g_pSpriteSurface[i]->m_gameTickOfLastUse)});
    for (int i=1; i<C_TILE_SCREEN_COUNT; ++i)
        if (g_tileScreens[i]) candidates.push_back({size_t(C_MAX_SPRITES_ADDRESSABLE+i), dink_perf::age(now, g_tileScreens[i]->m_gameTickOfLastUse)});
    const auto sorted = dink_perf::oldestFirst(candidates, 2000);
    unsigned released = 0;
    for (const auto& e : sorted) {
        if (GetBaseApp()->GetMemUsed() <= low && GetBaseApp()->GetTexMemUsed() <= low) break;
        if (e.slot < C_MAX_SPRITES_ADDRESSABLE) { SAFE_DELETE(g_pSpriteSurface[e.slot]); }
        else { SAFE_DELETE(g_tileScreens[e.slot-C_MAX_SPRITES_ADDRESSABLE]); }
        if (++released == 64) break;
    }
    if (!released) nextScan = scanClock + 1000;
    if (SwitchScreenLoadProfile::Enabled())
        LogMsg("[cache] released=%u CPU=%d texture=%d target=%d", released,
            GetBaseApp()->GetMemUsed(), GetBaseApp()->GetTexMemUsed(), low);
    return;
#endif
...
#ifdef __SWITCH__
        || GetAudioManager()->GetLastMusicID() == AUDIO_HANDLE_BLANK
#endif
    )
	{
...
bool LoadSoundState(FILE *fp)
{
...
#ifdef __SWITCH__
    dink_music_diagnostics::write("RESTORE music=%s active=%d", g_dglo.m_lastMusicPath.c_str(), g_dglos.midi_active);
#endif
	...
	{
#ifdef __SWITCH__
        if (PlayMidi(g_dglo.m_lastMusicPath.c_str()))
...
static bool WriteStateToStream(FILE *fp)
{
	bool bOk = true;
	...
	if (bOk)
	{
		...
		bOk = bOk && SaveSoundState(fp);
	}

	return bOk && ferror(fp) == 0;
}

bool SaveState(string const &path, bool bSyncSaves)
{
    CreateDirectoryRecursively("", g_dglo.m_savePath);
#ifdef __SWITCH__
    const bool ok = dink_save::write(path, WriteStateToStream);
#else
    FILE *fp = fopen(path.c_str(), "wb");
    if (!fp) return false;
    const bool written = WriteStateToStream(fp);
    const bool closed = fclose(fp) == 0;
    const bool ok = written && closed;
#endif
    if (!ok) LogError("Unable to save state: %s", path.c_str());
    if (ok && bSyncSaves) SyncPersistentData();
    return ok;
}
...
void SaveStateWithExtra()
{
	...
	bool bPT = (GetApp()->GetVarWithDefault("language", uint32(1))->GetUINT32() == 1);
	const bool saved = SaveState(DinkGetSavePath()+"quicksave.dat");
	ShowQuickMessage(saved ? (bPT ? "Jogo salvo." : "State saved.") : (bPT ? "Falha ao salvar." : "Save failed."));
};
...
#ifdef __SWITCH__
    CreateDirectoryRecursively("", g_dglo.m_savePath);
    const bool saved = dink_save::commit(DinkGetSavePath(), {
        {"autosave.dat", WriteStateToStream},
        {"autosavedb.dat", [&db](FILE* fp) { return db.SaveToStream(fp); }}
    });
#else
    const bool stateSaved = SaveState(DinkGetSavePath()+"autosave.dat");
    const bool saved = stateSaved && db.Save(DinkGetSavePath()+"autosavedb.dat", false);
#endif
    bool bPT = (GetApp()->GetVarWithDefault("language", uint32(1))->GetUINT32() == 1);
    ShowQuickMessage(saved ? (bPT ? "(Salvamento automatico)" : "(Auto-saved)") : (bPT ? "Falha no salvamento." : "Auto-save failed."));
    if (!saved) LogError("Autosave failed in %s", DinkGetSavePath().c_str());
}
...
void LoadStateWithExtra(string forcedFileName)
{
	...
	LoadState(fName, false);
	bool bPT = (GetApp()->GetVarWithDefault("language", uint32(1))->GetUINT32() == 1);
	ShowQuickMessage(bPT ? "Jogo carregado." : "State loaded.");
