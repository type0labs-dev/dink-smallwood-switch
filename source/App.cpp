...
#include "GUI/LanguageSelectMenu.h"
...
//	pProvider->CacheShowMoreApps();
	...
	*/
	...
	bool bFileExisted;
#ifdef __SWITCH__
    if (!dink_save::recover(GetSavePath()) || !dink_save::recover(GetSavePath()+"dink/")) {
        LogError("Save recovery failed; preserving files for recovery");
        return false;
    }
    for (const auto& dir : GetDirectoriesAtPath(GetSavePath()+"dmods/")) {
        if (!dink_save::recover(GetSavePath()+"dmods/"+dir)) return false;
    }
#endif
...
#if (defined(RTLINUX) || defined(PLATFORM_LINUX)) && !defined(__SWITCH__)
{
...
#elif defined(PLATFORM_OSX)
{
	...
	if (!m_bDidPostInit)
	{
		...
		if (GetApp()->GetVarWithDefault("language_chosen", uint32(0))->GetUINT32() == 0 &&
		    !AutoTesterIsActive())
		{
			LanguageSelectMenuCreate(pGUIEnt);
		}
		else
		{
...
#ifdef __SWITCH__
    SetFPSLimit(v == VIDEO_FPS_LIMIT_ON ? 30 : 60);
#else
    SetFPSLimit(60);
#endif
...
{
#ifdef __SWITCH__
    if (!dink_save::write(GetSavePath()+"save.dat", [this](FILE* fp) { return m_varDB.SaveToStream(fp); }))
        LogError("Settings save failed; previous settings preserved");
#else
...
void App::AddTextToLog(const char *tex, const char *filename)
	{
#ifdef __SWITCH__
        AppendStringToFile(filename, tex);
        return;
#endif
